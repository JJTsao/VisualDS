...
**考試階段 + 題組(選用)**:老師在**看板 `results.html`** 的「考試設定」選**階段**並勾選**練習題組 / 正式題組**:
- `關閉`(預設):全部章節開放(自己測試用)。
- `練習模式`:只開練習題組;**不計分、不寫看板/Sheet、可無限重做、不計時**(模擬練習,提早開放)。
- `正式考試`:只開正式題組;**計分、每關一次、計時**。
伺服器端強制:不在當前題組的章節一律擋下(`chapter-locked`),學生改網址也進不去。設定存 `config.json`。
